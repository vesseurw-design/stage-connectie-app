...
async function continueStudentInit() {
    try {
        let companyIds = [];
        if (currentStudent.company_assignments && Array.isArray(currentStudent.company_assignments) && currentStudent.company_assignments.length > 0) {
            companyIds = currentStudent.company_assignments.map(a => a.company_id).filter(Boolean);
        } else if (currentStudent.company_id) {
            companyIds = String(currentStudent.company_id).split(',').map(id => id.trim()).filter(Boolean);
        }
        ...
        if (companyIds.length > 0) {
            ...
            if (companiesData && companiesData.length > 0) {
                currentStudent.companiesList = companiesData;
                currentStudent.assignedCompanies = typeof getStudentCompanyAssignments === 'function' 
                    ? getStudentCompanyAssignments(currentStudent, companiesData)
                    : companiesData.map(c => ({ company_id: c.id, company_name: c.company_name, company: c, days: currentStudent.scheduled_days || [] }));
                
                currentStudent.Bedrijven = companiesData[0];

                const phoneCompanies = companiesData.filter(c => c.phone);
                if (phoneCompanies.length > 0) {
                    const primaryPhone = phoneCompanies[0].phone;
                    ...
                    if (phoneBlock && phoneLink && phoneText) {
                        phoneText.textContent = phoneCompanies.length > 1 
                            ? `Bel ${phoneCompanies[0].company_name}: ${primaryPhone}`
                            : `Bel stagebedrijf: ${primaryPhone}`;
                    ...
                    if (absentBlock && absentLink) {
                        absentLink.textContent = `Bel stagebedrijf (${primaryPhone})`;
                        absentLink.href = `tel:${primaryPhone}`;
...
function renderHeader() {
    ...
    let companyStr = 'Geen stagebedrijf gekoppeld';
    if (currentStudent.assignedCompanies && currentStudent.assignedCompanies.length > 0) {
        companyStr = currentStudent.assignedCompanies.map(a => {
            const daysStr = (a.days && a.days.length > 0) ? ` (${a.days.join(', ')})` : '';
            return `${a.company_name}${daysStr}`;
        }).join(' • ');
    } else if (currentStudent.Bedrijven?.company_name) {
        companyStr = currentStudent.Bedrijven.company_name;
    }

    const classInfo = currentStudent.class ? ` • Klas: ${currentStudent.class}` : '';
    document.getElementById('student-info').textContent = `${companyStr}${classInfo}`;
}
...
async function exportStudentPDF() {
    ...
    reportContainer.innerHTML = `
        <div style="border-bottom: 3px solid #7e22ce; padding-bottom: 12px; margin-bottom: 16px; display: flex; justify-content: space-between; align-items: center;">
            <div>
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 11px; table-layout: fixed;">
            <thead>
                <tr style="background-color: #f3f4f6; border-bottom: 2px solid #d1d5db; text-align: left;">
                    ...
                    <th style="padding: 8px 10px; width: 51%; font-weight: bold;">Werkzaamheden / Dagverslag</th>
                </tr>
            </thead>
            <tbody>
                ${data.map((r, idx) => {
                    ...
                    return `
                        <tr style="background-color: ${bg}; border-bottom: 1px solid #e5e7eb; page-break-inside: avoid;">
                            ...
                            <td style="padding: 8px 10px; vertical-align: top; line-height: 1.4; word-break: break-word; white-space: pre-wrap; color: ${r.notes ? '#111827' : '#9ca3af'}; font-style: ${r.notes ? 'normal' : 'italic'};">
